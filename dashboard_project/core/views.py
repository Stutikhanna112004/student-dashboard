...
class CsrfExemptSessionAuthentication(SessionAuthentication):
    """
    DRF runs its own CSRF check on top of Django middleware.
    For cross-origin API calls this double-check causes 403s even when
    CSRF_TRUSTED_ORIGINS is set correctly.  Skip the DRF-level one.
    """
...
def login_api(request):
    ...
    if user is not None:
        login(request, user)
        # Explicitly save so the session is written to DB before the response
...
def dashboard_api(request):
    ...
    # Belt-and-suspenders check (IsAuthenticated already handles 401)
...
class DashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({
            'enrollments': enrollments, 'total_courses': total_courses,
            'avg_grade': avg_grade, 'avg_attendance': avg_attendance,
